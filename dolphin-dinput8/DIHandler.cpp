...
DIACTION CDIHandler::m_rgGameAction[] =
{
	...
	{ CDIHandler::INPUT_DPAD_AXIS_UP,	DIPOV_ANY_2,			0, TEXT("DPad Up/Down"),		},
};
...
CDIHandler::InitInput(HWND _hWnd)
{
	...
	strncpy( m_diafGame.tszActionMap, "Dolphin Pad Plugin", strlen("Dolphin Pad Plugin") );
			...
			case E_DIUTILERR_TOOMANYUSERS:
				{
					...
					wsprintf( sz, TEXT("There are not enough devices attached to the ")          \
						...
						m_dwNumPlayers );
					MessageBox( m_hWnd, sz, TEXT("Too Many Users"), MB_OK | MB_ICONEXCLAMATION );
					break;
				}
			default:
...
CDIHandler::UpdateInput(void)
{
	...
	for( DWORD i=0; i<dwNumDevices; i++ )
	{
		...
		// Get the semantics codes for the game menu
